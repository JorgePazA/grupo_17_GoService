...
const userModelAseo = {
    ...
    createUser: function (user) {
        ...
        users.push(user)
        fs.writeFileSync(path.resolve(__dirname, './baseAseo.json'), JSON.stringify(users, null, 4), { encoding: 'utf8' });
        return 'Creado'
    },

    deleteUser: function (cedula) {
        const newDb = this.getUsers().filter((item) => item.cedula != cedula);
        fs.writeFileSync(path.resolve(__dirname, './baseAseo.json'), JSON.stringify(newDb, null, 4), { encoding: 'utf8' });
    },
